...
def base_mocks(monkeypatch):
    ...
    monkeypatch.setattr("app.collectors.gmail_poller.get_gmail_service", lambda **kw: mocks.service)
    monkeypatch.setattr(
...
def test_poll_gmail_mensaje_ya_procesado_skip(monkeypatch):
    ...
    monkeypatch.setattr("app.collectors.gmail_poller.get_gmail_service", lambda **kw: MagicMock())
    monkeypatch.setattr(
        "app.collectors.gmail_poller.list_candidate_messages",
        lambda *a, **kw: [_make_msg()],
    )
...
def test_poll_gmail_error_inesperado_wrapped(base_mocks):
    ...
    assert len(summary["errores"]) == 1
    err = summary["errores"][0]
    # error_code must be a clean category
    ...
    # UNKNOWN wraps the message with type prefix and truncation — verify it's categorised
