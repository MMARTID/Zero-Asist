...
ErrorCode = Literal[
    ...
    "PARSE_ERROR",   # JSON / response parse failure (transient, retryable)
...
class PipelineError(Exception):
    ...
    def from_exception(cls, e: Exception) -> "PipelineError":
        ...
        if "jsondecode" in type(e).__name__.lower() or "json" in msg.lower():
            return cls(
                code="PARSE_ERROR",
                message="La respuesta del modelo no es JSON válido (error de parsing).",
                _original=e,
            )
        return cls(
            code="UNKNOWN",
            message=f"Error inesperado: {type(e).__name__}: {msg[:120]}",
            _original=e,
        )
